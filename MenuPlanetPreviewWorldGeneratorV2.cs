...
[Header("Experimental Signed Basin Hydrology")]
    ...
    private void EnsureGpuDisplacementTexture()
    {
        ...
        ReleaseGpuDisplacementTexture();
        gpuDisplacementTexture = new RenderTexture(mapWidth, mapHeight, 0, RenderTextureFormat.RFloat, RenderTextureReadWrite.Linear)
        {
    ...
    {
        if (!logHeightAutoDebugAfterDispatch || gpuDisplacementTexture == null) return;
        ...
        int heightRtWidthSnapshot = gpuDisplacementTexture.width;
        int heightRtHeightSnapshot = gpuDisplacementTexture.height;
        ...
        AsyncGPUReadback.Request(gpuDisplacementTexture, 0, req =>
        {
            if (req.hasError)
            {
            ...
            var heightData = req.GetData<float>();
            float[] heightSnapshot = heightData.ToArray();
            ...
            for (int i = 0; i < count; i++)
            {
                float signedHeight = heightSnapshot[i];
                float hillMask = 0f;
                float mountainMask = 0f;
                float basinPotential = 0f;
