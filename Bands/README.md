...
For ambient actors, assign an Animator and a controller dedicated to that actor's action. For
example, campfire actors use a sitting controller while structure workers use a gathering or
crafting controller. The controller's default state defines the action; no category or variant
parameters are required. `BandCampAmbientActor` only randomizes normalized start time and applies
the configured small speed range. Create another controller when adding a distinct action rather
than adding action-selection parameters to a shared controller. Looping setup and all meshes,
controllers, clips, culture-specific prefabs, sockets, and pivots remain deliberate Unity-side
art work; no placeholder art is generated by the runtime system.
