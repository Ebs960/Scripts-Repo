...
For ambient actors, assign an Animator and choose Campfire, Shelter, or General. Animator
controllers may expose integer parameters named `CampIdleCategory` and `CampIdleVariant`
(names are configurable per actor). The component safely ignores absent parameters, chooses a
compatible variant once, randomizes normalized start time, and applies the configured small
speed range. Looping/blend-tree setup and all meshes, controllers, clips, culture-specific
prefabs, sockets, and pivots remain deliberate Unity-side art work; no placeholder art is
generated by the runtime system.
