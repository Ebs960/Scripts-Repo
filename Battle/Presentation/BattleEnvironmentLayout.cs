...
public static class BattleEnvironmentLayout
{
    ...
    public static List<BattleDecorationPlacement> Generate(BattleCell cell,BattleBiomeVisualProfile profile,int battleSeed,float radius,Vector2 riverDirection=default,Vector2 coastNormal=default)
    {
        ...
        Add(result,cell,profile,battleSeed,radius,BattleDecorationKind.Grass,profile.grassPrefabs,Count(profile.grassDensity*vegetation,profile.maximumGrassClumps),profile.grassScaleRange,false,riverDirection,coastNormal);
        Add(result,cell,profile,battleSeed,radius,BattleDecorationKind.Bush,profile.bushPrefabs,reserved?0:Count(profile.bushDensity*vegetation*(cell.IsForest?profile.forestBushMultiplier:1f),profile.maximumBushes),profile.bushScaleRange,true,riverDirection,coastNormal);
        Add(result,cell,profile,battleSeed,radius,BattleDecorationKind.Rock,profile.rockPrefabs,reserved?0:Count(profile.rockDensity*rocks,profile.maximumRocks),profile.rockScaleRange,true,riverDirection,coastNormal);
        Add(result,cell,profile,battleSeed,radius,BattleDecorationKind.Prop,profile.environmentalPropPrefabs,reserved?0:Count(profile.propDensity,profile.maximumProps),profile.propScaleRange,true,riverDirection,coastNormal);
        if(cell.HasSoftCover)Add(result,cell,profile,battleSeed,radius,BattleDecorationKind.SoftCover,profile.softCoverPrefabs,1,profile.bushScaleRange,true,riverDirection,coastNormal);
        if(cell.HasHardCover)Add(result,cell,profile,battleSeed,radius,BattleDecorationKind.HardCover,profile.hardCoverPrefabs,1,profile.rockScaleRange,true,riverDirection,coastNormal);
        if(cell.HasPort)Add(result,cell,profile,battleSeed,radius,BattleDecorationKind.Port,profile.portPrefabs,1,profile.propScaleRange,true,riverDirection,coastNormal);
        return result;
    }
