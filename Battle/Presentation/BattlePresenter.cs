...
public sealed class BattlePresenter : MonoBehaviour
{
    ...
    {if(!unitViews.TryGetValue(action.UnitId,out var attacker))yield break;unitViews.TryGetValue(action.TargetUnitId,out var defender);Vector3 target=defender!=null?defender.transform.position:Layout.GetCellCenter(action.TargetCell);attacker.Face(target);if(action.IsRanged){attacker.PlayAttack();yield return new WaitForSecondsRealtime(.2f);yield return StartCoroutine(PlayTracer(attacker.transform.position+Vector3.up*.35f,target+Vector3.up*.3f));}else yield return StartCoroutine(attacker.Lunge(target));
        ...
        if(action.CounterAttack!=null){var counter=action.CounterAttack;var counterEvent=new BattlePresentationEvent{Type=BattlePresentationEventType.CounterAttack,UnitId=counter.AttackerUnitId,TargetUnitId=counter.TargetUnitId,WeaponIndex=counter.WeaponIndex,IsRanged=counter.IsRanged,Died=counter.Damage?.Died??false};if(unitViews.TryGetValue(counter.AttackerUnitId,out var counterView)&&unitViews.TryGetValue(counter.TargetUnitId,out var counterTarget)){Vector3 counterPoint=counterTarget.transform.position;counterView.Face(counterPoint);if(counter.IsRanged){counterView.PlayAttack();yield return new WaitForSecondsRealtime(.2f);yield return StartCoroutine(PlayTracer(counterView.transform.position+Vector3.up*.35f,counterPoint+Vector3.up*.3f));}else yield return StartCoroutine(counterView.Lunge(counterPoint));if(counterEvent.Died)counterTarget.PlayDeath();else counterTarget.PlayHit();yield return new WaitForSecondsRealtime(counterEvent.Died ? .8f : .3f);counterTarget.FinishPresentation(counterEvent.Died);counterView.FinishPresentation();}}
        attacker.FinishPresentation(action.CounterAttack?.Damage?.Died??false);}

    private System.Collections.IEnumerator PlayTracer(Vector3 from,Vector3 to){var tracer=GameObject.CreatePrimitive(PrimitiveType.Sphere);temporaryEffects.Add(tracer);tracer.name="Presentation-only projectile fallback";tracer.transform.localScale=Vector3.one*.12f;var c=tracer.GetComponent<Collider>();if(c!=null)Destroy(c);float t=0f;while(t<1f){t+=Time.unscaledDeltaTime*5f;Vector3 p=Vector3.Lerp(from,to,t);p.y+=Mathf.Sin(Mathf.Clamp01(t)*Mathf.PI)*.8f;tracer.transform.position=p;yield return null;}temporaryEffects.Remove(tracer);Destroy(tracer);}
