...
public sealed class BattlePresenter : MonoBehaviour
{
    ...
    private void Refresh(){var session=manager?.ActiveBattle;if(session==null||Layout==null)return;RefreshOverlays();var live=new HashSet<int>();
        foreach(var unit in session.Units){if(unit==null)continue;live.Add(unit.UnitId);if(!unitViews.TryGetValue(unit.UnitId,out var view)||view==null){var go=new GameObject($"Tactical Unit {unit.UnitId}");go.transform.SetParent(unitsRoot,false);view=go.AddComponent<BattleUnitView>();view.Initialize(unit,manager?.TacticalCamera);unitViews[unit.UnitId]=view;}
    ...
    {if(!unitViews.TryGetValue(action.UnitId,out var attacker))yield break;unitViews.TryGetValue(action.TargetUnitId,out var defender);Vector3 target=defender!=null?defender.transform.position:Layout.GetCellCenter(action.TargetCell);attacker.Face(target);if(action.IsRanged){attacker.PlayAttack();yield return new WaitForSecondsRealtime(.2f);yield return StartCoroutine(PlayProjectile(attacker.Snapshot,action.WeaponIndex,action.AttackProfile,attacker.transform.position+Vector3.up*.35f,target+Vector3.up*.3f));}else yield return StartCoroutine(attacker.Lunge(target));
        ...
        if(action.CounterAttack!=null){var counter=action.CounterAttack;var counterEvent=new BattlePresentationEvent{Type=BattlePresentationEventType.CounterAttack,UnitId=counter.AttackerUnitId,TargetUnitId=counter.TargetUnitId,WeaponIndex=counter.WeaponIndex,IsRanged=counter.IsRanged,Died=counter.Damage?.Died??false};if(unitViews.TryGetValue(counter.AttackerUnitId,out var counterView)&&unitViews.TryGetValue(counter.TargetUnitId,out var counterTarget)){Vector3 counterPoint=counterTarget.transform.position;counterView.Face(counterPoint);if(counter.IsRanged){counterView.PlayAttack();yield return new WaitForSecondsRealtime(.2f);yield return StartCoroutine(PlayProjectile(counterView.Snapshot,counter.WeaponIndex,null,counterView.transform.position+Vector3.up*.35f,counterPoint+Vector3.up*.3f));}else yield return StartCoroutine(counterView.Lunge(counterPoint));if(counterEvent.Died)counterTarget.PlayDeath();else counterTarget.PlayHit();yield return new WaitForSecondsRealtime(counterEvent.Died ? .8f : .3f);counterTarget.FinishPresentation(counterEvent.Died);counterView.FinishPresentation();}}
        attacker.FinishPresentation(action.CounterAttack?.Damage?.Died??false);}

    private System.Collections.IEnumerator PlayProjectile(BattleUnitSnapshot snapshot,int weaponIndex,BattleAttackProfile specialProfile,Vector3 from,Vector3 to)
    {
        BattleProjectileVisual visual=BattleProjectileVisualResolver.Resolve(snapshot,weaponIndex,specialProfile);
        if(visual.TravelType==BattleProjectileTravelType.Beam){var beam=GameObject.CreatePrimitive(PrimitiveType.Cube);Track(beam,"Presentation-only beam");Vector3 delta=to-from;beam.transform.SetPositionAndRotation((from+to)*.5f,Quaternion.LookRotation(delta));beam.transform.localScale=new Vector3(specialProfile!=null?.08f:.035f,specialProfile!=null?.08f:.035f,delta.magnitude);yield return new WaitForSecondsRealtime(specialProfile!=null?.18f:.1f);DestroyEffect(beam);}
        ...
        if(visual.ImpactPrefab!=null){var impact=Instantiate(visual.ImpactPrefab,to,Quaternion.identity);Track(impact,$"Tactical impact ({visual.ImpactPrefab.name})");impact.transform.localScale*=specialProfile!=null?1.5f:1f;yield return new WaitForSecondsRealtime(specialProfile!=null?.35f:.2f);DestroyEffect(impact);}
    }
    private void Track(GameObject effect,string effectName){effect.name=effectName;temporaryEffects.Add(effect);foreach(var collider in effect.GetComponentsInChildren<Collider>(true))collider.enabled=false;foreach(var flight in effect.GetComponentsInChildren<LaunchedProjectile>(true))flight.enabled=false;foreach(var body in effect.GetComponentsInChildren<Rigidbody>(true)){body.isKinematic=true;body.detectCollisions=false;body.linearVelocity=Vector3.zero;body.angularVelocity=Vector3.zero;}}
