...
Shader "Space Graphics Toolkit/Terrain Planet"
{
   Properties
   {
	...
	[Header(BIOME ARRAYS)]
	[Toggle(_BIOME_ARRAYS)] _HasBiomeArrays ("	Use Biome Arrays", Float) = 0
	[NoScaleOffset]_BiomeAlbedoArray("	Biome Albedo Array", 2DArray) = "" {}
	[NoScaleOffset]_BiomeNormalArray("	Biome Normal Array", 2DArray) = "" {}
	[NoScaleOffset]_BiomeIndexMap("	Biome Index Map", 2D) = "white" {}
	_BiomeAlbedoArray_Depth("	Biome Texture Count", Float) = 1
...
ZWrite On
   ...
   #pragma fragment Frag
	...
	#pragma shader_feature_local _BIOME_ARRAYS
...
#define _USINGTEXCOORD2 1
	...
	{
		#if _BIOME_ARRAYS
			// Sample biome index from the biome index map
			float biomeIndex = SAMPLE_TEXTURE2D(_BiomeIndexMap, sampler_BiomeIndexMap, d.texcoord0.xy).r;
			
			// Convert from normalized (0-1) to actual array index
			float arrayIndex = biomeIndex * (_BiomeAlbedoArray_Depth - 1.0);
			
			// Sample from biome arrays
			float3 biomeAlbedo = SAMPLE_TEXTURE2D_ARRAY(_BiomeAlbedoArray, sampler_BiomeAlbedoArray, d.texcoord0.xy, arrayIndex).rgb;
			float3 biomeNormal = UnpackNormal(SAMPLE_TEXTURE2D_ARRAY(_BiomeNormalArray, sampler_BiomeNormalArray, d.texcoord0.xy, arrayIndex));
			
			o.Albedo     = ShiftColor(biomeAlbedo, _AlbedoShift) * _Color.rgb;
			o.Normal     = biomeNormal;
