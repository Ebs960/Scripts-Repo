...
public class MenuPlanetPreviewHydrologyMaskGenerator : MonoBehaviour
{
    ...
    public void GenerateNow()
    {
        ...
        float[] land = new float[size], h = new float[size], mountain = new float[size], shelf = new float[size], river = new float[size], lake = new float[size], flow = new float[size], wetland = new float[size], depth = new float[size];
        ...
        var sources = PickSources(land, h, mountain, shelf);
        int reachedCoast = 0;
    ...
    private List<int> PickSources(float[] land,float[] h,float[] mountain,float[] shelf){int target=waterwaysPreset==0?16:waterwaysPreset==1?34:68; target=Mathf.RoundToInt(target*Mathf.Lerp(0.35f,1f,moisture)); var c=new List<int>(); for(int y=16;y<hydrologyMaskHeight-16;y+=3) for(int x=0;x<hydrologyMaskWidth;x+=3){int i=y*hydrologyMaskWidth+x; if(land[i]<0.6f||h[i]<0.35f||shelf[i]>0.2f) continue; float moistureFactor=Mathf.Lerp(0.4f,1f,moisture); float s=h[i]*0.65f+mountain[i]*0.25f+moistureFactor*0.1f; s*=0.9f+0.2f*Hash01(x,y); if(s>0.5f) c.Add(i);} c.Sort((a,b)=>h[b].CompareTo(h[a])); var outp=new List<int>(); foreach(var i in c){bool near=false; int x=i%hydrologyMaskWidth,y=i/hydrologyMaskWidth; foreach(var o in outp){int ox=o%hydrologyMaskWidth, oy=o/hydrologyMaskWidth; if(Mathf.Abs(ox-x)+Mathf.Abs(oy-y)<24){near=true;break;}} if(!near){outp.Add(i); if(outp.Count>=target)break;}} return outp; }
