Shader "Custom/MenuPlanetPreview"
{
    Properties
    {
        ...
        _UseTectonicPreview("Use Tectonic Preview", Float) = 1
        _UseExperimentalSignedTerrain("Use Experimental Signed Terrain", Float) = 0
        _ExperimentalNegativeDisplacementScale("Experimental Negative Displacement Scale", Float) = 0.45
        _ShowSignedHeightOnly("Show Signed Height Only", Float) = 0
        _ShowBasinPotentialOnly("Show Basin Potential Only", Float) = 0
        _ShowSelectedBasinMaskOnly("Show Selected Basin Mask Only", Float) = 0
        _ShowExperimentalRiverPathOnly("Show Experimental River Path Only", Float) = 0
                _ShowTectonicLandMaskOnly("Show Tectonic Land Mask Only", Float) = 0
        _ShowTectonicHeightOnly("Show Tectonic Height Only", Float) = 0
        _ShowPlateBoundariesOnly("Show Plate Boundaries Only", Float) = 0
        _ShowConvergentBoundariesOnly("Show Convergent Boundaries Only", Float) = 0
        _ShowDivergentBoundariesOnly("Show Divergent Boundaries Only", Float) = 0
        _ShowMountainUpliftOnly("Show Mountain Uplift Only", Float) = 0
        _ShowGeneratedHillReliefOnly("Show Generated Hill Relief Only", Float) = 0
        ...
        _ShowContinentalPotentialOnly("Show Continental Potential Only", Float) = 0
            ...
            _ShowSurfaceWaterMaskOnly("Show Surface Water Mask Only", Float) = 0
    ...
    SubShader
    {
        ...
        HLSLINCLUDE
            ...
            float2 GetTectonicUV(float3 objNorm)
            {
                float3 dir = normalize(objNorm);
            ...
            float4 GetGpuHeightData(float3 objNorm)
            {
                float2 uv = GetTectonicUV(objNorm);
            ...
            {
                float terrainHeight = GetGpuHeightData(objNorm).r;
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float edge = (_UseTectonicPreview > 0.5) ? GetTectonicSurface(objNorm).r : smoothstep(_LandThreshold - 0.04, _LandThreshold + 0.04, n);
                ...
                if (_ShowMountainUpliftOnly > 0.5) return float4(((_UseTectonicPreview > 0.5) ? gpuHeightData.b : tectonicSurface.b).xxx, 1.0);
                if (_ShowGeneratedHillReliefOnly > 0.5) return float4(((_UseTectonicPreview > 0.5) ? gpuHeightData.g : saturate(tectonicCrust.a * 1.5)).xxx, 1.0);
                ...
                float generatedHydrologyWetness = 0.0;
                if (_UseTectonicPreview > 0.5)
                {
                    float2 waterwayUVEarly = GetTectonicUV(objNorm);
                    float4 generatedWaterwayMaskEarly =
                        SAMPLE_TEXTURE2D(_WaterwayMaskTex, sampler_WaterwayMaskTex, waterwayUVEarly);

                    float generatedRiverEarly = generatedWaterwayMaskEarly.r;
                    float generatedLakeEarly = generatedWaterwayMaskEarly.g;
                    float generatedWetnessEarly = generatedWaterwayMaskEarly.b;

                    generatedHydrologyWetness =
                        max(
                            generatedWetnessEarly,
                            max(generatedRiverEarly, generatedLakeEarly) * 0.65
                        );
                ...
                float oceanSurfaceWaterMask = saturate(1.0 - edge);
                ...
                smoothnessMask = lerp(smoothnessMask, marshSmooth, marshWeight * dryLandSurfaceMask);
                smoothnessMask = lerp(smoothnessMask, taigaSmooth, taigaWeight * dryLandSurfaceMask);
                smoothnessMask = lerp(smoothnessMask, mountainSmooth, mtnBlend * dryLandSurfaceMask);
                smoothnessMask = lerp(smoothnessMask, oceanSmooth, totalSurfaceWaterMask * (1.0 - infernalWeight));
                ...
                surfN = normalize(lerp(surfN, normalize(input.normalWS + biomeN), edge * dryLandSurfaceMask * _BiomeNormalStrength * (_UseTextureDrivenBiomes > 0.5 ? 1.0 : 0.0)));
                surfN = normalize(lerp(surfN, normalize(input.normalWS + mtnN), mtnBlend * dryLandSurfaceMask * _MountainNormalStrength));
                float oceanWaveStrength = lerp(_OceanNormalStrength * 0.55, _OceanNormalStrength, oceanSurfaceWaterMask);
                surfN = normalize(lerp(surfN, normalize(input.normalWS + oceanN), totalSurfaceWaterMask * oceanWaveStrength * (1.0 - infernalWeight)));
                ...
                float waterSpecMask = totalSurfaceWaterMask * (1.0 - infernalWeight);
